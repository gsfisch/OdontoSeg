...
from util.variables import get_class_weigths
...
def quadratic_weighted_kappa(logits, labels, min_rating=None, max_rating=None):
    """
    ...
    rater_a = predicted.view(1,-1).tolist()
    rater_b = labels.view(1,-1).tolist()
  
    rater_a = np.array(rater_a, dtype=int)[0]
    rater_b = np.array(rater_b, dtype=int)[0]
    assert(len(rater_a) == len(rater_b))
    if min_rating is None:
    ...
    for i in range(num_ratings):
        for j in range(num_ratings):
            ...
            result += numerator / max(0.0000000001, denominator)

    cudaNumerator = Variable(torch.tensor(numerator), requires_grad=True).cuda()
    cudaDenominator = Variable(torch.tensor(denominator), requires_grad=True).cuda()

    return 1.0 - cudaNumerator / cudaDenominator
