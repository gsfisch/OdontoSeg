...
from Architecture.MyArchitecture_pvt_v2_b1__U_Net.Encoder import ConvBlock
...
class Decoder(nn.Module):
    def __init__(self, bottleneck_channels=1152, skip_channels=[96, 192, 384, 576], n_classes=4):
        ...
        self.seg_head = nn.Sequential(
            UpSampling(skip_channels[0], skip_channels[0]),
            nn.Conv2d(skip_channels[0], skip_channels[0], kernel_size=3, padding=1),

            UpSampling(skip_channels[0], skip_channels[0]),
            nn.Conv2d(skip_channels[0], skip_channels[0], kernel_size=3, padding=1),

            nn.Conv2d(skip_channels[0], n_classes, kernel_size=1),
