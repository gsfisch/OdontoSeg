...
from new_data import get_data
from util.data import UNET_Dataset
...
class LRFinder(object):
    ...
    def _train_batch(self, inputs, labels):
        ...
        loss = GD(idc=[0,1,2])(outputs, labels, None) + SurfaceLoss(idc=[1, 2])(outputs, labels, None)
    ...
    def plot(self, skip_start=0, skip_end=0, log_lr=True, show_lr=True):
        ...
        plt.savefig('books_read_' + str(randint(1, 200)) + '.png')
...
import pandas as pd
from util.utils import save_df, plot_loss
from loops import *
import segmentation_models_pytorch as smp
from optimizers.main import *
import time
params = {'batch_size': 2, 'shuffle': True, 'num_workers': 0}
model = torch.load('./experiments/experimento_28/model.pth').cuda()
opt = optmizer(option='rangerlars', model=model, lr=1e-8, weight_decay=1e-4)
AUGMENTATIONS_TRAIN, AUGMENTATIONS_VALID = create_augmentations()
x_train, y_train, x_valid, y_valid = get_data('./data-train')

dataTrain = UNET_Dataset(x_train, y_train, AUGMENTATIONS_TRAIN)
dataValid = UNET_Dataset(x_valid, y_valid, AUGMENTATIONS_VALID)
training_generator = torch.utils.data.DataLoader(dataset=dataTrain, **params)  

lr_finder = LRFinder(model, opt, None, device="cuda")
